...
export default function Card(beaconVal: any) { // changed to use json instead of string
  ...
  const actualValue = beaconVal.beaconVal; //what is this bro
    ...
    const upvote = async () => {
      try {
        const response = await fetch(URL + "/beacons/:" + actualValue.id + "/upvote", {
          method: 'POST',
          headers: {
    ...
    const downvote = async () => {
      try {
        const response = await fetch(URL + "/beacons/:" + actualValue.id + "/downvote", {
          method: 'POST',
          headers: {
          'Content-Type': 'application/json'
        }
      ...
      return (
        <div className="bg-white/70 backdrop-blur-lg shadow-lg border border-gray-200 rounded-xl p-6 text-black transition-transform duration-300 hover:-translate-y-2 w-[350px]">
            ...
            <div className="mt-4 flex justify-around items-center">
                <button className="flex items-center gap-1 px-3 py-1.5 rounded-md bg-green-500 text-white hover:bg-green-600 transition" onClick={upvote}>
                    <FaRegArrowAltCircleUp className="text-lg" />

                </button>
                <button className="flex items-center gap-1 px-3 py-1.5 rounded-md bg-red-500 text-white hover:bg-red-600 transition" onClick={downvote}>
